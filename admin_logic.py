...
# 1. التحقق من بيانات المالك (علي محجوب)
def verify_admin_credentials(u, p):
    # استعلام مؤندكس وسريع
...
def get_dashboard_data(filter_type=None):
    ...
    if filter_type == 'pending':
        return query.filter_by(is_active=False).order_by(Vendor.id.desc()).all()
    return query.order_by(Vendor.id.desc()).all()

# 4. دالة القبول السيادي (تفعيل + محفظة + أول حركة في كشف الحساب)
def approve_vendor_logic(vendor_id):
    ...
    try:
        ...
        # ب- إنشاء المحفظة بنظام MAH
        if not vendor.wallet:
            new_wallet = Wallet(vendor_id=vendor.id)
            db.session.add(new_wallet)
            db.session.flush() # للحصول على ID المحفظة فوراً
            
            # ج- تسجيل الحركة الكاملة (بداية كشف الحساب)
...
# 5. إضافة مورد يدوياً من برج المراقبة
def create_vendor_logic():
    ...
    try:
        ...
        # إنشاء محفظة وحركة أولية للمورد اليدوي
        w = Wallet(vendor_id=new_v.id)
        db.session.add(w)
        db.session.flush()
        
        tx = Transaction(wallet_id=w.id, trans_type="تأسيس", amount=0.0, balance_after=0.0, description="تسجيل يدوي من الإدارة")
        db.session.add(tx)
